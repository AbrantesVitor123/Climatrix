...
export function ResultsDashboard({ show, onClose, scenario }: ResultsDashboardProps) {
  if (!show) return null;
      ...
      <div className="max-w-7xl mx-auto">
        <header className="flex justify-between items-start mb-6 gap-4">
            <div className="flex-grow">
                ...
                <p className="text-lg text-sky-300 mt-1">{scenario.name}</p>
            </div>
            <div className="flex items-start gap-4">
                <div 
                    className="w-56 h-32 md:w-64 md:h-36 flex-shrink-0 cursor-pointer transition-transform duration-200 hover:scale-105"
                    onClick={onClose}
                    title="Voltar ao mapa"
                >
                    <MapThumbnail scenario={scenario} />
                </div>
                <button onClick={onClose} className="text-gray-400 hover:text-white hover:bg-gray-700 rounded-full p-2 transition-colors self-start" aria-label="Fechar painel de resultados"><CloseIcon /></button>
            </div>
        </header>
